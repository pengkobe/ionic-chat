...
UserSchema.statics.loadFriends = function(username, cb) {
  this.findOne({ username: username }).populate("friends").exec(cb);
};
...
UserSchema.statics.addResponse_friendsDoc = function(userid, friendid, cb) {
  ...
  this.findOne(query).exec(function(err, doc) {
 ...
 */
UserSchema.statics.updateResponse_friendDoc = function(
  userid,
  friendid,
  state,
  cb
 ...
 * @param {Object} state 状态[ 0:待确认 | 1:成为好友 ]
 */
UserSchema.statics.updateRequset_friendsDoc = function(
  userid,
  friendid,
  state,
  cb
 ...
 * @param {Function} cb 回调函数
 */
UserSchema.statics.addResponse_groupDoc = function(
  userid,
  friendid,
  groupid,
  cb
 ...
 * @param {Object} state 状态[ 0:待确认 | 1:成为好友 ]
 */
UserSchema.statics.updateResponse_groupDoc = function(
  userid,
  friendid,
  groupid,
  state,
  ...
  this.findOne(query, function(err, user) {
    ...
    for (var i = 0; i < user.response_groups.length; i++) {
      if (
        user.response_groups[i].from == friendid &&
        user.response_groups[i].groupid == groupid
      ) {
 ...
 * @param {Function} cb 回调函数
 */
UserSchema.statics.addRequset_groupsDoc = function(
  userid,
  rawfriendid,
  rawgroupid,
  cb
 ...
 * @param {Object} state 状态[ 0:待确认 | 1:成为好友 ]
 */
UserSchema.statics.updateRequset_groupDoc = function(
  userid,
  friendid,
  groupid,
  state,
  ...
  this.findOne(query, function(err, user) {
    ...
    if (user && user.requset_groups) {
      for (var i = 0; i < user.requset_groups.length; i++) {
        if (
          user.requset_groups[i].to == friendid &&
          user.requset_groups[i].groupid == groupid
        ) {
