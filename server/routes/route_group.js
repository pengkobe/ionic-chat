...
exports.createGroup = function(req, res) {
  ...
  var GroupEntity = new GroupModel({
    groupname: groupname,
    members: [userid],
    groupimg: groupimg
  });
    ...
    if (user && user.length == 0) {
      ...
      UserModel.populate(user, opts, function(err, populatedDocs) {
        ...
        if (populatedDocs && populatedDocs.response_groups) {
          var ret = [];
          var response_groups = populatedDocs.response_groups;
          for (var i = 0; i < response_groups.length; i++) {
            if (response_groups[i].state == 0) {
              ret.push(response_groups[i]);
            }
          }
          res.json(ret);
        } else {
...
exports.addgroupmember = function(req, res) {
  ...
  UserModel.addRequset_groupsDoc(userid, friendid, groupid, function(err, raw) {
    if (err) {
      log4js.err(err);
      // todo
      res.json(err);
...
exports.res_addgroupmember = function(req, res) {
  ...
  // 更新回复状态
  UserModel.updateResponse_groupDoc(userid, friendid, groupid, state, function(
    err,
    doc
  ) {
    if (err) {
  ...
  // 更新请求状态
  UserModel.updateRequset_groupDoc(friendid, userid, groupid, state, function(
    err,
    doc
  ) {
    if (err) {
